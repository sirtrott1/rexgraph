...
def difference_tensor(ref, inp, *, ref_labels=None, inp_labels=None, verify=True, matching="auto"):
    """Return the exact native C1 difference and its dimension readings.

    Vertex coordinates are the full union. Stable relation IDs are used when
    both endpoints carry them; otherwise anonymous support matching preserves
    primary slot multiplicities and pairs identical orientations first.

    Witness differences may have nonzero column sums. This does not indicate
    a broken boundary or alignment. The difference is a linear operator, not
    necessarily a canonical primary relation or a chain map. Numerical SciPy
    compatibility is an explicit call to the result's as_scipy method.

    The legacy verify argument remains accepted, but cannot disable exact
    endpoint, label or coordinate validation.
    """
    from rexgraph.boundary_difference import boundary_difference
    if not isinstance(verify, (bool, np.bool_)):
        raise TypeError("verify must be a boolean")
    result = boundary_difference(ref, inp, ref_labels=ref_labels,
                                 other_labels=inp_labels, matching=matching)
