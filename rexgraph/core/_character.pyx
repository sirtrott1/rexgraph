...
cdef void _compute_phi_core(const f64* B1, const f64* B1_RLp,
    ...
    for k in range(nhats):
        # tmp = B1_RLp @ hat_k
        bl_gemm_nn(B1_RLp, hat_data[k], tmp_buf, nV, nE, nE)
        # phi[:, k] = diag(tmp @ B1_RLp^T) / S0_diag
        for v in range(nV):
            # S0[v,v] = B1_RLp[v,:] . B1[v,:]
            s0_vv = 0
            for e in range(nE):
                s0_vv += B1_RLp[v * nE + e] * B1[v * nE + e]
            if fabs(s0_vv) > 1e-15:
                phi_vk = 0
                for e in range(nE):
                    phi_vk += tmp_buf[v * nE + e] * B1_RLp[v * nE + e]
...
def compute_phi_from_b1rlp(np.ndarray[f64, ndim=2] B1,
    ...
    cdef f64* hat_ptrs[8]
    cdef int k
    cdef np.ndarray[f64, ndim=2] hat_arr_b
    for k in range(nhats):
        hat_arr_b = hats[k]
        hat_ptrs[k] = &hat_arr_b[0, 0]
    _compute_phi_core(&B1[0, 0], &B1_RLp_c[0, 0],
                       <const f64* const*>hat_ptrs,
                       &phi[0, 0], &tmp_buf[0, 0], nV, nE, nhats)
    return phi
