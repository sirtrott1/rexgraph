...
**File:** `_laplacians.pyx` (678 lines)
...
**File:** `_faces.pyx` (773 lines)

Classifies faces into proper and self-loop types, filters B2 for exact Hodge
decomposition, extracts per-face descriptors, and computes structural metrics
relating faces to vertices and edges.
...
**File:** `_fiber.pyx` (280 lines)

Computes pairwise similarity between edges (via chi cosine) and between
vertices (via phi similarity and fiber bundle similarity). Also builds
threshold graphs from similarity matrices and projects simplex coordinates
to 3D for visualization.
...
restriction/lifting, and per-edge energy decomposition.
