...
def quotient_verify_chain(np.ndarray[f64, ndim=2] B1_quot,
    ...
    # Sparse matmul; inspect only stored nonzeros (no dense nE_q x nF_q product).
    import scipy.sparse as sp
    B1s = sp.csr_matrix(np.asarray(B1_quot, dtype=np.float64))
    B2s = sp.csc_matrix(np.asarray(B2_quot, dtype=np.float64))
    prod = (B1s @ B2s).tocoo()
    cdef f64 max_abs = float(np.abs(prod.data).max()) if prod.nnz else 0.0
...
def relative_betti(np.ndarray[f64, ndim=2] B1_quot,
    ...
    if nE_quot > 0 and nF_quot > 0:
        rank_B2 = _sparse_rank(sp.csc_matrix(np.asarray(B2_quot, dtype=np.float64)))

    b0 = nV_quot - rank_B1
    b1 = nE_quot - rank_B1 - rank_B2
    b2 = nF_quot - rank_B2
...
def relative_cycle_basis(np.ndarray[f64, ndim=2] B1_quot,
    ...
    import scipy.sparse as sp
    B1q = np.asarray(B1_quot, dtype=np.float64)
    ...
    # dense eigendecomposition (the retained oracle) so the result is always exact.
    from rexgraph.graded_boundary import _sparse_rank
    rank_b1 = _sparse_rank(sp.csc_matrix(B1q)) if nE > 0 and B1q.shape[0] > 0 else 0
    rank_b2 = _sparse_rank(sp.csc_matrix(B2q)) if has_faces else 0
    cdef Py_ssize_t beta1_rel = nE - rank_b1 - rank_b2

    if beta1_rel <= 0:
        return np.zeros((nE, 0), dtype=np.float64)

    from rexgraph.harmonic_sparse import harmonic_basis_from_boundaries
    B2_arg = sp.csr_matrix(B2q) if has_faces else None
    H = harmonic_basis_from_boundaries(sp.csc_matrix(B1q), B2_arg)
...
def congruence_classes_edges(np.ndarray[f64, ndim=2] B1,
    ...
    """
    return _congruence_partition(B1, e_mask, tol)
...
def congruence_residual(np.ndarray[f64, ndim=2] B1,
                        int edge_a, int edge_b,
                        np.ndarray[i32, ndim=1] basis_edges):
    """
    Compute the continuous congruence residual between two edges.

    Returns the L2 norm of the projection residual when projecting
    (col_a - col_b) of B1 onto the span of the basis edge columns.
    ...
    edge_a, edge_b : int
        Indices of the two edges to compare.
    basis_edges : (n_basis,) int32
    ...
    """
    cdef np.ndarray[f64, ndim=1] diff = B1[:, edge_a] - B1[:, edge_b]
    cdef int n_basis = basis_edges.shape[0]

    if n_basis == 0:
        return float(np.linalg.norm(diff))

    cdef np.ndarray[f64, ndim=2] basis_mat = np.empty(
        (B1.shape[0], n_basis), dtype=np.float64)
    cdef int j
