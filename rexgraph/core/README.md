...
The relational complex V -> E -> F defines three Hodge Laplacians:
...
relational complex V -> E -> F, verifies the chain condition B1 @ B2 = 0, and
...
R^nV + R^nE + R^nF. D encodes the full relational complex in a single real
...
- `face_overlap_K2(B2, nE, nF)` -> f64[nF, nF]
  ...
  to the boundary size of face f. Computed as a scipy SPARSE matmul over the
  face incidence (|B2| > 0.5)^T (|B2| > 0.5) in O(nnz), never the
  O(nF^2 * nE) dense pair scan.
...
- `similarity_complex(similarity, n, threshold)` -> dict
  ...
  Builds a full relational complex from a thresholded similarity matrix. Calls
...
- `linkage_complex(sfb, threshold, n_entities, face_fill='clique')` -> dict
  ...
  - `'cycle'`: faces are the fundamental cycle basis (arbitrary-arity n-gon
    ...
    key giving each face's boundary size is added.

  Any other `face_fill` value raises ValueError.

  Returns: src, tgt, weights, n_edges, nV, nF, B1 (f64[nV, nE]),
  B2 (f64[nE, nF]), beta (beta_0, beta_1, beta_2), triangles (i32[nF, 3]), plus
  `face_lengths` (i32[nF]) in `'cycle'` mode.
...
- `field_schrodinger_evolve(psi_E, psi_F, evals_RL1, evecs_RL1, evals_L2, evecs_L2, t, B1=None)` -> (psi_E_t, psi_F_t, psi_V_t)
  ...
  B1 when provided. Dense oracle for `graph.py`'s `evolve_field_wave` (which
  routes to `scale_propagator.schrodinger_apply`; see the oracle note below).
...
Container for signals on the relational complex. Holds f0 (vertices),
...
- `harmonic_content_all(Bvoid, evals_L1, evecs_L1, n_voids, nE)` -> f64[n_voids]
  ...
  Harmonic content for all voids. Dense oracle: needs the L1 eigenbasis.
...
- `relative_betti(B1_quot, B2_quot, tol=1e-10)` -> (beta0_rel, beta1_rel, beta2_rel)
  ...
  Relative Betti numbers from EXACT integer ranks of the quotient boundary
  operators via `graded_boundary._sparse_rank` (rational column reduction, no
  SVD).
...
joins produce valid relational complexes (B1j @ B2j = 0 guaranteed because
...
- `build_interfacing_bundle(target_indices, target_weights, vertex_weights, B1, evals_L0, evecs_L0, L_O, L_SG, evals_RL, evecs_RL, target, nV, nE)` -> dict
  ...
  Dense ORACLE for `graph.py`'s `interfacing_vector`, which at scale routes to
  `sparse_interfacing.build_interfacing_bundle_sparse` (see the module note
  above).
