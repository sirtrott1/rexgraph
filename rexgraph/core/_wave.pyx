...
def amplitude_graded_projection(B1, B2,
    ...
    endpoints (any arity):
        ...
        psi_0(v)       = a_v
        psi_0(nV+e)    = (prod_{v in endpoints(e)} |a_v|)^(1/deg(e)) * sign(B1[first,e])
    ...
    # Read edge endpoints from B1's signed CSC columns (arbitrary arity).
    B1c = B1.tocsc() if sp.issparse(B1) else sp.csc_matrix(np.asarray(B1, dtype=np.float64))
    cdef np.ndarray[long, ndim=1] indptr = np.asarray(B1c.indptr, dtype=np.int64)
    cdef np.ndarray[long, ndim=1] indices = np.asarray(B1c.indices, dtype=np.int64)
    cdef np.ndarray[f64, ndim=1] data = np.asarray(B1c.data, dtype=np.float64)
    cdef int v, e, k
    cdef long lo, hi, deg
    cdef f64 prod_abs, sgn, nm

    # Vertex component
    for v in range(nV):
        pv[v] = av[v]

    # Edge component: geometric mean over ALL of edge e's endpoints (any arity)
    for e in range(nE):
        lo = indptr[e]
        hi = indptr[e + 1]
        deg = hi - lo
    ...
    # Face component: B2^T psi_E (sparse matvec, O(nnz))
    if nF > 0 and B2 is not None:
        B2c = B2.tocsr() if sp.issparse(B2) else sp.csr_matrix(np.asarray(B2, dtype=np.float64))
        if B2c.shape[1] == nF:
            psi[nV + nE:nV + nE + nF] = np.asarray(B2c.T @ psi[nV:nV + nE]).ravel()

    # Normalize
    nm = sqrt(float(np.dot(psi, psi)))
    if nm > 1e-10:
        for v in range(NG):
            pv[v] /= nm
