...
def typed_face_selection(np.ndarray[i32, ndim=1] edge_types,
    ...
    cdef np.ndarray[i32, ndim=1] realized_edges = np.empty(3 * nF_r, dtype=np.int32)
    cdef np.ndarray[f64, ndim=1] realized_signs = np.empty(3 * nF_r, dtype=np.float64)
    cdef np.ndarray[i32, ndim=1] realized_types = np.empty(nF_r, dtype=np.int32)
    cdef np.ndarray[i32, ndim=1] void_edges = np.empty(3 * nF_v, dtype=np.int32)
    cdef np.ndarray[f64, ndim=1] void_signs = np.empty(3 * nF_v, dtype=np.float64)
    cdef i32[::1] re = realized_edges, ve = void_edges, rt = realized_types
    cdef f64[::1] rs = realized_signs, vs = void_signs
    cdef Py_ssize_t ri = 0, vi = 0, rti = 0
        ...
        for j_v in range(lo_v, hi_v):
            ...
            while p_u < hi_v and p_w < hi_w:
                if ai[p_u] < ai[p_w]:
                    ...
                    t_vw = et[e_vw]
                    if t_uv == t_uw and t_uw == t_vw:
                        re[ri] = e_uv;   rs[ri] = 1.0;  ri += 1
                        re[ri] = e_uw;   rs[ri] = -1.0; ri += 1
                        re[ri] = e_vw;   rs[ri] = 1.0;  ri += 1
                        rt[rti] = t_uv;  rti += 1
                    else:
                        ve[vi] = e_uv;   vs[vi] = 1.0;  vi += 1
                        ve[vi] = e_uw;   vs[vi] = -1.0; vi += 1
                        ve[vi] = e_vw;   vs[vi] = 1.0;  vi += 1
                    p_u += 1
                    p_w += 1

    return {
        'realized_edges': realized_edges,
        'realized_signs': realized_signs,
        'nF_realized': int(nF_r),
        'void_edges': void_edges,
        'void_signs': void_signs,
        'nF_void': int(nF_v),
        'face_types': realized_types,
...
def clique_enumeration(np.ndarray[i32, ndim=1] src,
    ...
    cliques = {k: [] for k in range(2, max_k + 1)}

    # Enumerate all cliques by extending partial cliques with
    # vertices that are adjacent to every vertex in the current clique.
    # Only consider vertices with index > max(R) to avoid duplicates.
    def extend(R, candidates):
        k = len(R)
        if k >= 2 and k <= max_k:
            cliques[k].append(tuple(sorted(R)))
        if k >= max_k:
            return
        for i, v in enumerate(candidates):
            # New candidates: vertices in candidates[i+1:] adjacent to v
            new_cands = [u for u in candidates[i+1:] if u in adj[v]]
            extend(R + [v], new_cands)

    # Start with each active vertex
    active = set()
    for e in range(nE):
        active.add(int(src[e]))
        active.add(int(tgt[e]))
    active_sorted = sorted(active)

    for i, v in enumerate(active_sorted):
        candidates = [u for u in active_sorted[i+1:] if u in adj[v]]
        extend([v], candidates)
