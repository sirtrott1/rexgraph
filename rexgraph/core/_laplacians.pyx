...
cdef object _as_scipy(obj):
    ...
    (from _sparse), any scipy sparse, or a dense ndarray."""
...
def build_L0_sparse(B1_in):
    """L_0 = B_1 B_1^T = D - A as scipy CSR (nnz ~ nV + 2*nE). Never densifies B_1."""
    B1s = _as_scipy(B1_in)
    return (B1s @ B1s.T).tocsr()


def build_L1_down_sparse(B1_in):
    """L_1^{down} = B_1^T B_1 as scipy CSR (sparse matmul)."""
    B1s = _as_scipy(B1_in)
    return (B1s.T @ B1s).tocsr()


def build_L1_up_sparse(B2_in):
    """L_1^{up} = B_2 B_2^T as scipy CSR. Zero (nE x nE empty) when there are no faces."""
    B2s = _as_scipy(B2_in)
    return (B2s @ B2s.T).tocsr()


def build_L2_sparse(B2_in):
    """L_2 = B_2^T B_2 as scipy CSR (face Laplacian)."""
    B2s = _as_scipy(B2_in)
    return (B2s.T @ B2s).tocsr()
...
def extract_diag_L1(B1_in, B2_in):
    ...
    def _tocsr(A):
        if _sp.issparse(A):
            return A.tocsr()
        if hasattr(A, "row_ptr") or hasattr(A, "indptr"):
            try:
                from rexgraph.core._sparse import to_scipy_csr
                return to_scipy_csr(A).tocsr()
            except Exception:
                pass
        return _sp.csr_matrix(np.asarray(A, dtype=np.float64))

    B1 = _tocsr(B1_in)
    B2 = _tocsr(B2_in)
    dd = np.asarray(B1.multiply(B1).sum(axis=0), dtype=np.float64).ravel()   # col sum sq
    du = np.asarray(B2.multiply(B2).sum(axis=1), dtype=np.float64).ravel()   # row sum sq
    return dd, du
