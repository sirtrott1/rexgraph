...
class TestVoidBoundary:
    ...
    def test_b1_bvoid_zero(self, k4_partial):
        ...
        assert n_voids == 1
        Bv = Bvoid.toarray() if hasattr(Bvoid, "toarray") else np.asarray(Bvoid)
        product = np.asarray(k4_partial.B1, dtype=np.float64) @ Bv
    ...
    def test_bvoid_entries_pm1(self, k4_partial):
        ...
        Bvoid, _, _ = _void.build_void_boundary(
            ...
            k4_partial.nV, k4_partial.nE)
        Bv = Bvoid.toarray() if hasattr(Bvoid, "toarray") else np.asarray(Bvoid)
        for col in range(Bv.shape[1]):
            nonzero = np.abs(Bv[:, col]) > 0.5
            assert nonzero.sum() == 3
            assert np.allclose(np.abs(Bv[nonzero, col]), 1.0)
...
class TestRexGraphIntegration:
    ...
    def test_harmonic_content_eigenfree_fallback(self, k4_partial):
        ...
        g = k4_partial
        vc = g.void_complex
        Bv, nv = vc['Bvoid'], vc['n_voids']
        assert nv > 0
        sb = g.spectral_bundle
        dense = _void.harmonic_content_all(Bv, sb['evals_L1'], sb['evecs_L1'], nv, g.nE)
        sparse = _void.harmonic_content_all_sparse(g.B1, g.B2_hodge, Bv, nv, g.nE)
        np.testing.assert_allclose(sparse, dense, atol=1e-9)
        # the fallback path (no spectrum passed) yields the same, not NaN
        res = _void.build_void_complex(
            g.B1, g.B2_hodge, *g._adjacency_bundle, g.nV, g.nE, None, None, 0, None, None)
        assert not np.any(np.isnan(res['eta']))
        np.testing.assert_allclose(res['eta'], dense, atol=1e-9)
    ...
    def test_lvoid_stays_sparse(self, k4_partial):
        ...
        vc = k4_partial.void_complex
        Lv = vc['Lvoid']
        assert sp.issparse(Lv)
        Bv = vc['Bvoid']
        assert np.allclose(np.asarray(Lv.todense()), np.asarray((Bv @ Bv.T).todense()))
        assert np.isclose(float(Lv.diagonal().sum()), vc['void_strain'])
