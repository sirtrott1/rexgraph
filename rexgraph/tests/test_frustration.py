...
def _manual_L_SG(nV, nE, src, tgt, signs, vertex_weights):
    """Reference implementation of the documented object K_s = B1^T W B1.

    B1[v,e] = -1 if v is the source of e, +1 if v is the target, and the
    edge sign multiplies the column. So each (vertex, edge) incidence has
    ...
    def bsign(v, e):
        return (1.0 if v == tgt[e] else -1.0) * signs[e]
    ...
    for v in range(nV):
        ...
        for a in range(len(edges)):
            for b in range(a + 1, len(edges)):
                ...
                contrib = wv * bsign(v, ei) * bsign(v, ej)
                Ks[ei, ej] += contrib
                Ks[ej, ei] += contrib
...
class TestBuildLSG:
    ...
    def test_diagonal_equals_offdiag_abs_sum(self, tri):
        ...
        L = _frustration.build_L_SG(nV, nE, src, tgt)
        diag = np.diag(L)
        offdiag_abs_sum = np.abs(L - np.diag(diag)).sum(axis=1)
        assert np.allclose(diag, offdiag_abs_sum, atol=1e-10)
