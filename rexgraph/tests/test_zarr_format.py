"""
Tests for rexgraph.io.zarr_format -- Zarr-based storage.

Requires zarr. Skipped if zarr is not installed.

Verifies:
    - save/load roundtrip: RexGraph reconstructed with correct nV/nE/nF
    - Array save/load: save_zarr_array / load_zarr_array
    - Cache groups: algebra, topology, spectral written and readable
    - TemporalRex roundtrip
    - Container API: write_to_group / read_from_group / list_groups
    - Weighted graph roundtrip: w_E preserved
"""
import os
import tempfile

import numpy as np
import pytest

try:
    import zarr
    HAS_ZARR = True
except ImportError:
    HAS_ZARR = False

pytestmark = pytest.mark.skipif(not HAS_ZARR, reason="zarr not installed")

if HAS_ZARR:
    from rexgraph.io.zarr_format import (
        RexZarrFormat,
        save_zarr,
        load_zarr,
        save_zarr_array,
        load_zarr_array,
    )

from rexgraph.graph import RexGraph


# Fixtures

@pytest.fixture
def k4():
    return RexGraph.from_simplicial(
        sources=np.array([0, 0, 0, 1, 1, 2], dtype=np.int32),
        targets=np.array([1, 2, 3, 2, 3, 3], dtype=np.int32),
        triangles=np.array([[0, 1, 2], [0, 1, 3], [0, 2, 3], [1, 2, 3]], dtype=np.int32),
    )


@pytest.fixture
def triangle():
    return RexGraph.from_graph([0, 1, 0], [1, 2, 2])


@pytest.fixture
def zarr_path(tmp_path):
    return str(tmp_path / "test.zarr")


# Basic Roundtrip

class TestRexGraphRoundtrip:

    def test_basic(self, k4, zarr_path):
        save_zarr(zarr_path, k4)
        loaded = load_zarr(zarr_path)
        assert isinstance(loaded, RexGraph)
        assert loaded.nV == k4.nV
        assert loaded.nE == k4.nE
        assert loaded.nF == k4.nF

    def test_betti_preserved(self, k4, zarr_path):
        save_zarr(zarr_path, k4)
        loaded = load_zarr(zarr_path)
        assert loaded.betti == k4.betti

    def test_directed_flag(self, triangle, zarr_path):
        save_zarr(zarr_path, triangle)
        loaded = load_zarr(zarr_path)
        assert loaded._directed == triangle._directed

    def test_weighted(self, tmp_path):
        """Edge weights preserved through roundtrip."""
        w = np.array([1.0, 2.0, 3.0], dtype=np.float64)
        rex = RexGraph.from_graph([0, 1, 0], [1, 2, 2], w_E=w)
        path = str(tmp_path / "weighted.zarr")
        save_zarr(path, rex)
        loaded = load_zarr(path)
        assert np.allclose(loaded._w_E, w)


# Array Save/Load

class TestArrayIO:

    def test_1d(self, zarr_path):
        arr = np.array([1.0, 2.0, 3.0])
        save_zarr_array(arr, zarr_path)
        loaded = load_zarr_array(zarr_path)
        assert np.allclose(loaded, arr)

    def test_2d(self, zarr_path):
        arr = np.array([[1, 2], [3, 4]], dtype=np.float64)
        save_zarr_array(arr, zarr_path)
        loaded = load_zarr_array(zarr_path)
        assert np.allclose(loaded, arr)

    def test_complex(self, zarr_path):
        arr = np.array([1+2j, 3+4j], dtype=np.complex128)
        save_zarr_array(arr, zarr_path)
        loaded = load_zarr_array(zarr_path)
        assert np.allclose(loaded, arr)


# Cache Groups

class TestCacheGroups:

    def test_topology_cache(self, k4, zarr_path):
        save_zarr(zarr_path, k4, cache=["topology"])
        # Should not error; cache is written alongside core data
        loaded = load_zarr(zarr_path)
        assert loaded.nV == k4.nV

    def test_all_cache(self, triangle, zarr_path):
        """cache='all' writes without error."""
        save_zarr(zarr_path, triangle, cache="all")
        loaded = load_zarr(zarr_path)
        assert loaded.nE == triangle.nE


# TemporalRex Roundtrip

class TestTemporalRexRoundtrip:

    def test_basic(self, zarr_path):
        from rexgraph.graph import TemporalRex
        snaps = [
            (np.array([0, 1, 0], dtype=np.int32),
             np.array([1, 2, 2], dtype=np.int32)),
            (np.array([0, 1, 0, 1], dtype=np.int32),
             np.array([1, 2, 2, 3], dtype=np.int32)),
        ]
        trex = TemporalRex(snaps)
        save_zarr(zarr_path, trex)
        loaded = load_zarr(zarr_path)
        assert isinstance(loaded, TemporalRex)
        assert loaded.T == 2


# Container API

class TestContainerAPI:

    def test_multi_object(self, tmp_path):
        path = str(tmp_path / "container.zarr")
        fmt = RexZarrFormat()
        r1 = RexGraph.from_graph([0, 1], [1, 2])
        r2 = RexGraph.from_graph([0, 1, 0], [1, 2, 2])
        fmt.write_to_group(path, "graph_a", r1)
        fmt.write_to_group(path, "graph_b", r2)
        names = fmt.list_groups(path)
        assert "graph_a" in names
        assert "graph_b" in names
        loaded_a = fmt.read_from_group(path, "graph_a")
        loaded_b = fmt.read_from_group(path, "graph_b")
        assert loaded_a.nE == 2
        assert loaded_b.nE == 3


# RexZarrFormat Class

class TestRexZarrFormat:

    def test_compressor_none(self, k4, zarr_path):
        fmt = RexZarrFormat(compressor="none")
        fmt.write(zarr_path, k4)
        loaded = fmt.read(zarr_path)
        assert loaded.nV == k4.nV

    def test_overwrite(self, k4, triangle, zarr_path):
        """Writing to same path overwrites."""
        save_zarr(zarr_path, k4)
        save_zarr(zarr_path, triangle)
        loaded = load_zarr(zarr_path)
        assert loaded.nE == triangle.nE
