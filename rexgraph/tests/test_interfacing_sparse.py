...
from rexgraph.sparse_interfacing import (
    build_interfacing_bundle_oracle,
...
def test_full_bundle_parity(fixture, request):
    ...
    for ti, tw, target in _cases(rex):
        ...
        sparse = build_interfacing_bundle_oracle(rex, ti, tw, target)
...
def test_keys_and_shapes(k4):
    ...
    assert b['iv'] is None and b['sphere_pos'] is None
    assert b['channel_direction'].shape == (3,)
...
def test_forced_sparse_path_eigenfree_parity():
    ...
    assert sparse['schrodinger'] is None
    assert sparse['coverage'] is None
    assert sparse['mode_diagnostics']['status'] == 'not-requested'
