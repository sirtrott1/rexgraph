...
class TestAmplitudeGradedProjectionArity:
    ...
    def test_two_arity_geometric_mean(self):
        from rexgraph.graph import RexGraph
        g = RexGraph.from_graph(np.array([0, 1, 2, 0]), np.array([1, 2, 0, 2]))
        amps = np.array([1.0, 2.0, 3.0])
        psi = _wave.amplitude_graded_projection(
            np.asarray(g.B1, dtype=float), np.asarray(g.B2_hodge, dtype=float),
            amps, g.nV, g.nE, int(g.nF_hodge))
        # each edge amplitude ∝ sqrt(|a_i a_j|)·sign(B1[smallest,e]); check the ratio
        B1 = np.asarray(g.B1, dtype=float)
        raw = np.zeros(g.nE)
        for e in range(g.nE):
            rows = np.nonzero(np.abs(B1[:, e]) > 0.5)[0]
            i, j = rows[0], rows[1]
            raw[e] = np.sqrt(abs(amps[i] * amps[j])) * (1.0 if B1[i, e] > 0 else -1.0)
        edge_block = psi[g.nV:g.nV + g.nE]
        # equal up to the global normalization constant
        scale = raw[0] / edge_block[0]
    ...
    def test_branching_edge_uses_all_endpoints(self):
        ...
        scale = -geo3 / edge[0]
        np.testing.assert_allclose(edge[0] * scale, -geo3, atol=1e-12)
        np.testing.assert_allclose(edge[1] * scale, geo2, atol=1e-12)  # edge1 sign +
