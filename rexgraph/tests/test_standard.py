...
class TestClusteringValues:
    ...
    def test_triangle_is_one(self):
        # K3: adjacency 0-1,0-2,1-2 ; every vertex clustering coefficient = 1.0
        adj_ptr = np.array([0, 2, 4, 6], dtype=np.int32)
        adj_idx = np.array([1, 2, 0, 2, 0, 1], dtype=np.int32)
        cc = _standard.clustering(adj_ptr, adj_idx, 3)
    ...
    def test_k4_is_one(self):
        ...
        assert np.allclose(cc, 1.0), f"K4 clustering must be 1.0, got {cc}"

    def test_paw_fractional(self):
        # triangle {0,1,2} + pendant edge 0-3.
        # v0: nbrs {1,2,3}, 1 edge among them -> 2*1/(3*2)=1/3
        # v1,v2: nbrs form an edge -> 1.0 ; v3: deg 1 -> 0
        adj_ptr = np.array([0, 3, 5, 7, 8], dtype=np.int32)
        adj_idx = np.array([1, 2, 3, 0, 2, 0, 1, 0], dtype=np.int32)
        cc = _standard.clustering(adj_ptr, adj_idx, 4)
        assert np.allclose(cc, [1.0 / 3.0, 1.0, 1.0, 0.0]), \
            f"paw clustering wrong: {cc}"
