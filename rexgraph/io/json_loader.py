...
def _load_rexgraph_json(data: dict) -> Any:
    ...
    if "B2_col_ptr" in data:
        ...
        if "B2_vals" in data:
            # Face orientation signs as serialized by to_json.
            kw["B2_vals"] = np.array(data["B2_vals"], dtype=np.float64)
        else:
            # Legacy/hand-written JSON with no orientation: unsigned faces.
...
def _load_adjacency_json(
    ...
    # Honor threshold (matches load_matrix_csv): |weight| <= threshold pruned.
    if threshold > 0:
        matrix = matrix.copy()
