...
CONTRACT_NOTES = {
    "COORDINATE_APPLY": "Exact action through declared named coordinates without relabelling them as cell chains.",
    "OPERATION_DELTA": "Exact operation and field innovation channels under independent input and output correspondences.",
    "INJECTION_DELTA": "Exact realization change and aligned source amplitude innovation. Injectivity is not presumed.",
    "WORD_DELTA": "Exact position indexed defects of a retained operation word, with all interaction moments.",
    "KERNEL_MOMENTS": "Evaluate a retained rational moment kernel on its named input field.",
    ...
    "ACCESSION_DELTA": "Core exact accession defect with optional independent output correspondence K. Without K the existing ambient and named output conventions remain. Explicit current endpoint declarations, no inferred correspondence or time division.",
    ...
    "PARTICIPATION_WALK": "Imported: the relation mediated participation walk U De^-1 U^T Dv^-1 of Zhou, Huang and Schoelkopf through U = abs(B1) W on C0, which reads relation weights as intensities; exact Q action and observable transpose, uniform dangling columns. Formerly MARKOV_VIEW.",
    ...
    "PAGERANK_SOLVE": "Imported: the PageRank fixed point of Page and Brin on an explicit PARTICIPATION_WALK, solved over Q by a sparse augmented system with an exact residual certificate. Replaced relationally by RESOLVENT_RANK. Formerly PAGERANK_EXACT.",
    "PAGERANK_ITERATION": "Imported: the PageRank fixed point by power iteration on an explicit PARTICIPATION_WALK with uniform or supplied C0 restart, a measured L1 error bound and refusal on exhaustion; no exact claim. Replaced relationally by RESOLVENT_RANK. Formerly PAGERANK.",
    "RESOLVENT_RANK": "The relational replacement for PageRank: (I + lam L_k)^-1 seed with lam = damping/(1 - damping) at a declared grade, exact over Q through the native Green action, with no transition matrix, operator or inverse. Under metric 'walk' at grade 0 on pair relations it is exactly personalized PageRank. At higher grades it reads faces through B_(k+1), and where harmonic directions exist (a hole, or the tail differences of a branching relation) it conserves mass and is signed. Metric policies 'declared', 'walk', 'degree' and 'completed'; explicit per grade metrics or a field calculus supply metrics read from fields.",
    ...
    "FIELD_DELTA": "Exact B'J-JB and transpose boundary defects applied to a canonical Q Chain, with explicit GradedMap correspondence and identity or declared positive rational endpoint metrics. Returns named target coordinate records, not fields on the old Rex. No lineage or time correspondence is inferred.",
    "FIELD_DELTA_MOMENT": "Sum of down and up correspondence defect quadrances in identity or declared target metrics, over all supplied block columns. Not simple field subtraction or a derivative.",
    "ORIENTED_FIELD_DELTA_MOMENT": "Down defect quadrance minus up defect quadrance, with FIELD_DELTA's explicit correspondence and endpoint metric scope. May be negative. A chain map alone does not imply a zero up defect.",
    ...
    "HARMONIC_LOG": "Float logarithm of EFFECTIVE_MODES, the collision entropy of the normalized spectrum of the same sector. The exact count is the carrier; a sector with no modes is refused.",
    ...
    "HEAD": "Returns the exact C0 Chain head mask, not a Cell. The head is the participant carrying the -1, which is slot zero unless the relation declares another one.",
    "STAR": "Returns a GradedCellPattern; grades are not flattened into a CellSet.",
    "SHARE": "Returns a C0 Chain with exact rational share coefficients: 1/(k-1) on each tail unless the relation declares its own share vector, which the chain then returns as declared.",
    ...
    "ORIENTATION": "Primary C1 distinguished participant mask, as HEAD, including a declared head; witnesses and self loops have no head and return zero masks.",
    ...
    'MOLECULAR_INFO': 'Recorded atom, bond, source and condition declarations for one molecule.',
    'MOLECULAR_FIELD': 'Exact typed molecular property field with optional native bond injection.',
    'MOLECULAR_CONFORMER': 'Supplied rational atom coordinates retaining their unit and reference frame.',
    'CONFORMATION_FIELD': 'Selected exact quadrances, spreads and oriented determinant readings.',
    'CONFORMATION_DIRECTION': 'Exact derivative in a declared coordinate direction.',
    'MOLECULAR_DELTA': 'Located atom and bond changes under explicit identity correspondence.',
    'PROCESS_COMPARE': 'Retained comparison of supplied pathway samples on one named schedule.',
    'FACTOR_CONTRAST': 'Signed finite response and secant for an explicitly declared factor step.',
    "SECTION_SYSTEM": "Exact incidence equations retaining local coordinate spaces and source identities.",
    "SECTION_FIELD": "An explicitly supplied or assigned section with retained tensor axes.",
    "SECTION_RESIDUAL": "Located anchor residuals in the declared mediator coordinates.",
    "SECTION_COMPLETE": "All compatible sections under explicit exact observations.",
    "SECTION_OBSERVE": "An observation of an affine section family with its free directions.",
    "SECTION_VALUE": "A field only when the observation is invariant over its section family.",
    "SECTION_RECIPE": "A finite durable declaration of section spaces and restriction maps.",
    "SECTION_RESTORE": "Bind a stored section declaration to its exact source versions and permissions.",
    "SECTION_DELTA": "Restriction and section changes at explicitly aligned endpoints.",
