...
# generic package named tests collides when all five suites run together.
_AGENT_ROOT = str(pathlib.Path(__file__).resolve().parents[1])
if os.environ.get("REXGRAPH_TEST_INSTALLED") != "1" and _AGENT_ROOT not in sys.path:
    sys.path.insert(0, _AGENT_ROOT)
_fixtures = types.ModuleType("_rexgraph_agent_tests")
_fixtures.__path__ = [str(pathlib.Path(__file__).resolve().parent)]
sys.modules["_rexgraph_agent_tests"] = _fixtures
# Spawned processes need an importable test helper module. This path contains
# fixtures only, not a source copy of any installed runtime package.
if _fixtures.__path__[0] not in sys.path:
    sys.path.insert(0, _fixtures.__path__[0])
