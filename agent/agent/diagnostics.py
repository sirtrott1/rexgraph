...
built, there is no pure-Python fallback: construction (e.g. ``RexGraph(...)``)
still succeeds, but the analytic paths that need the compiled kernels
(e.g. ``.betti``) raise ``ModuleNotFoundError`` at call time (audit 3.2).
